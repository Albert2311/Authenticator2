...
//     $secret = $Authenticator->generateRandomSecret();
//     $_SESSION['auth_secret2'] = $secret;
// }
// $privatekey2 = $_SESSION['auth_secret2'];
//test end
...
if (!isset($_SESSION['failed'])) {
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        if ($emailCheckResult) {
            if (mysqli_num_rows($emailCheckResult) > 0) {
                $status = "Chưa xác minh";
                $updateQuery = "UPDATE users SET status = '$status', code = '$code' WHERE email = '$email'";
                $updateResult = mysqli_query($conn, $updateQuery);
                if ($updateResult) {
                    $subject = 'Mã xác minh email';
                    $message = "Mã xác minh của bạn là $code";
    ...
    // xác thực otp lúc nhập email để đổi password
if(isset($_POST['verifyEmail'])){
    ...
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $update_status = "Đã xác minh";
            $update_code = 0;
            $newQuery = "UPDATE users SET status = '$update_status', code = $update_code WHERE code = '$OTPverify'";
...
//
// redend otp resending when change password
if (isset($_POST['resendingPass'])) {
    if(!isset($_SESSION['email'])){
        $errors['ErrorEmail'] = "Không tìm thấy email";
    }else{
        //test
        $secret = $Authenticator->generateRandomSecret();
        $_SESSION['auth_secret'] = $secret;
        $privatekey2 = $_SESSION['auth_secret'];
        //end test
        $email = $_SESSION['email'];
        // $_SESSION['email'] = $email;
